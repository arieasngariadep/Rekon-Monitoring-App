...
class BniReleaseImport implements ToCollection, WithHeadingRow, WithCalculatedFormulas, WithEvents, WithValidation, SkipsOnError, SkipsOnFailure
{
    ...
    */
    public function collection(Collection $rows)
    {
        ...
        foreach ($rows as $row) {
            $data = BniModel::where('id', $row['id'])
            ->update([
                'bank_name_release' => $row['bank_name_release'],
    ...
    public function rules(): array
    {
        return [
             ...
             '*.matchingkey' => ['required', Rule::exists('bni', 'matchingkey')],
             '*.cust_name_release' => function($attribute, $value, $onFailure) {
                if ($value == NULL) {
                    $onFailure('CUST NAME RELEASE Kosong');
                }
              }, 
              '*.act_release' => function($attribute, $value, $onFailure) {
                if ($value == NULL) {
                    $onFailure('ACT RELEASE Kosong');
                }
            },
            '*.tanggal_reproses_payment' => function($attribute, $value, $onFailure) {
                if ($value == NULL) {
                    $onFailure('TANGGAL REPROSES PAYMENT Kosong');
                }
            },
            '*.id' => function($attribute, $parameters, $onFailure) {
                $bni = BniModel::find($parameters);
                if ($bni->status == 'CLOSED-DONE') {
